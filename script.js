const noteheadPath = "M 158 -100 C 202 -100 236 -75 236 -34 C 236 35 166 100 78 100 C 32 100 0 74 0 34 C 0 -34 87 -100 158 -100 Z";
const sharpPath = "M 9 93 L 30 85 C 37 83 41 68 41 62 V -12 C 41 -22 35 -27 30 -27 L 9 -19 C 8 -18 7 -18 5 -18 C 2 -18 0 -20 0 -24 V -81 C 0 -86 4 -92 9 -95 C 9 -95 26 -101 32 -103 C 37 -106 41 -115 41 -122 V -235 C 41 -240 45 -244 54 -244 C 60 -244 65 -240 65 -235 V -135 C 65 -131 69 -121 76 -121 L 77 -122 H 78 L 127 -142 C 131 -143 135 -149 135 -159 V -271 C 135 -275 141 -279 149 -279 C 154 -279 159 -275 159 -271 V -172 C 159 -168 163 -159 169 -159 C 169 -159 170 -159 171 -160 C 178 -163 189 -167 190 -168 H 192 C 197 -168 199 -166 199 -161 V -104 C 199 -99 195 -93 189 -91 C 189 -91 173 -84 168 -83 C 163 -80 159 -73 159 -64 V 15 C 159 23 164 29 172 29 C 177 25 189 21 190 20 H 192 C 197 20 199 22 199 27 V 84 C 199 89 195 95 189 97 C 189 97 173 103 169 105 C 164 108 159 116 159 127 V 235 C 159 240 154 244 146 244 C 141 244 135 240 135 235 V 139 C 135 133 133 123 125 123 C 112 126 85 137 75 143 C 68 148 65 158 65 166 V 270 C 65 275 60 279 52 279 C 45 279 41 275 41 270 V 173 C 41 166 35 162 29 162 C 25 163 9 169 9 169 C 8 170 7 170 5 170 C 2 170 0 168 0 164 V 107 C 0 102 4 96 9 93 Z M 65 -37 C 63 -30 61 -7 61 14 C 61 36 63 56 65 60 C 66 63 71 65 77 65 C 97 65 133 48 135 34 C 137 28 137 0 137 -25 C 137 -44 137 -62 135 -65 C 134 -67 130 -69 125 -69 C 107 -69 69 -53 65 -37 Z";
const naturalPath = "M 86 142 L 74 146 C 73 147 72 147 71 147 C 68 147 65 144 65 140 V -261 C 65 -267 69 -271 74 -271 H 85 C 90 -271 94 -267 94 -261 V -142 C 94 -136 100 -133 108 -133 C 131 -133 176 -153 187 -157 C 188 -158 189 -158 190 -158 L 192 -159 C 197 -159 199 -157 199 -152 V 260 C 199 266 195 270 189 270 H 179 C 174 270 169 266 169 260 V 127 C 169 123 166 123 161 123 C 141 123 89 141 89 141 S 88 142 86 142 Z M 169 28 V -54 C 169 -57 165 -59 160 -59 C 140 -59 94 -41 94 -26 V 56 C 94 59 97 60 101 60 C 121 60 169 39 169 28 Z";
const flatPath = "M 28 125 C 21 117 18 -340 18 -340 C 19 -354 32 -362 43 -362 C 51 -362 58 -357 58 -349 C 58 -333 53 -141 52 -123 C 52 -117 56 -112 61 -109 H 65 C 69 -109 78 -116 83 -120 C 92 -125 101 -129 110 -131 C 116 -133 121 -133 127 -133 C 164 -131 199 -102 199 -57 C 199 -20 174 29 103 79 C 83 92 64 114 40 127 C 40 127 37 129 35 129 C 32 129 30 128 28 125 Z M 56 54 C 56 58 57 66 64 66 C 81 66 123 18 130 3 C 140 -13 144 -30 144 -45 C 144 -64 134 -91 111 -91 C 93 -91 60 -67 55 -52 C 54 -49 53 -39 53 -26 C 53 6 56 54 56 54 Z";
const trebleClefPath = "M 301 -532 L 321 -416 C 323 -402 323 -402 344 -402 C 462 -402 537 -311 537 -209 C 537 -118 483 -50 402 -18 C 391 -14 390 -14 392 -5 C 401 45 415 121 415 166 C 415 302 311 326 258 326 C 137 326 106 248 106 196 C 106 146 138 104 191 104 C 240 104 268 142 268 186 C 268 232 239 254 214 261 C 196 266 189 269 189 274 C 189 285 210 298 253 298 C 300 298 380 283 380 165 C 380 127 368 58 358 4 C 357 -6 355 -5 346 -3 C 330 0 310 2 291 2 C 95 2 0 -130 0 -270 C 0 -431 123 -546 238 -641 C 247 -649 246 -650 245 -660 C 239 -693 234 -745 234 -791 C 234 -874 253 -974 312 -1040 C 328 -1058 353 -1078 364 -1078 C 373 -1078 394 -1056 406 -1038 C 438 -990 458 -922 458 -852 C 458 -724 392 -633 306 -547 C 301 -542 299 -542 301 -532 Z M 376 -954 C 327 -954 270 -878 270 -753 C 270 -727 271 -700 274 -679 C 276 -669 279 -668 285 -673 C 348 -729 424 -799 424 -889 C 424 -933 402 -954 376 -954 Z M 289 -410 L 272 -512 C 270 -521 269 -522 261 -515 C 163 -438 64 -351 64 -216 C 64 -105 154 -27 294 -27 C 310 -27 326 -29 339 -31 C 351 -34 352 -34 350 -42 L 310 -281 C 309 -290 307 -290 294 -287 C 246 -274 214 -239 214 -194 C 214 -157 238 -122 272 -108 C 278 -106 284 -103 284 -98 S 281 -89 274 -89 C 269 -89 259 -91 253 -94 C 198 -112 161 -163 161 -235 C 161 -303 206 -366 277 -390 C 291 -395 291 -394 289 -410 Z M 344 -282 L 383 -54 C 385 -45 386 -45 396 -50 C 441 -72 471 -113 471 -163 C 471 -234 418 -288 353 -294 C 343 -294 342 -292 344 -282 Z"

var main = document.getElementsByClassName("main")[0]
var notehead = document.createElementNS("http://www.w3.org/2000/svg", "path");
var flat = document.createElementNS("http://www.w3.org/2000/svg", "path");
var sharp = document.createElementNS("http://www.w3.org/2000/svg", "path");
var natural = document.createElementNS("http://www.w3.org/2000/svg", "path");
var trebleClef = document.createElementNS("http://www.w3.org/2000/svg", "path");
notehead.setAttribute("d", noteheadPath);
flat.setAttribute("d", flatPath);
natural.setAttribute("d", naturalPath);
sharp.setAttribute("d", sharpPath);
trebleClef.setAttribute("d", trebleClefPath);


document.getElementsByName("accidentals")[0].checked = true;

var currentCol = 0;
const cols = ['#540D6E', '#c42623', '#087E8B', 'white']
function colChange(col) {
    document.querySelector(':root').style.setProperty('--maincol', cols[col]);
    let e = document.getElementsByClassName("colors")[0];
    e.children[currentCol].classList.remove("selected-col");
    currentCol = col;
    e.children[col].classList.add("selected-col");
}

function clear() {
    main.innerHTML = "";
}

function generate() {
    clear();
    if (document.getElementById("noOfNotes").value < 1) {
        document.getElementById("noOfNotes").value = 1
    }
    let len = parseInt(document.getElementById("noOfNotes").value);

    let probs = document.getElementsByClassName("slider");
    let probSum = 0;
    for (let i = 0; i < 12; i++) {
        probSum += +probs[i].value;
    }
    let melody = [];
    for (let i = 0; i < len; i++) {
        let draw = Math.floor(Math.random() * probSum);
        for (let j in probs) {
            if (draw <= probs[j].value) {
                melody.push(+j);
                break;
            } else {
                draw -= probs[j].value;
            }
        }
    }
    
    console.log(melody);
    start();
    for (let i in melody) {
        staff();
        note(melody[i]);
    }
    end();
    adjustHeight(len + 2);
}

flatDist = [3, 2, 2, 1, 1, 0, 0, -1, -2, -2, -3, -3];
sharpDist = [3, 3, 2, 2, 1, 1, 0, -1, -1, -2, -2, -3];

function note(p) {
    // From 0 = F to 11 = E
    let unit = main.lastElementChild;
    let head = notehead.cloneNode(false);

    // 0 = flats, 1 = sharps, 2 = both
    let accidentals = 0;
    for (let i = 0; i < 3; i++) {
        if (document.getElementsByName("accidentals")[i].checked) {
            accidentals = +i;
            break;
        }
    }
    if (accidentals == 2) {
        accidentals = [0, 1][Math.round(Math.random())];
    }

    let acc;
    let hasAcc = [1, 3, 5, 8, 10].includes(p);
    if (accidentals == 0) {
        acc = flat.cloneNode(false);
        acc.style.transform = `translate(300%, calc(279% * (${flatDist[p]} / 3)))`;
        head.style.transform = `translate(600%, calc(279% * (${flatDist[p]} / 3)))`;
    } else {
        acc = sharp.cloneNode(false);
        acc.style.transform = `translate(300%, calc(279% * (${sharpDist[p]} / 3)))`;
        head.style.transform = `translate(600%, calc(279% * (${sharpDist[p]} / 3)))`;
    }

    
    unit.appendChild(head);
    if (hasAcc) {
        unit.appendChild(acc);
    }
    
}

function staff() {
    let temp = document.getElementById("blank-staff");
    temp.classList.add("unit");
    main.appendChild(temp.content.cloneNode(true));
}

function start(clef="treble") {
    let temp = document.getElementById("start");
    temp.classList.add("unit");
    main.appendChild(temp.content.cloneNode(true));
}

function end() {
    let temp = document.getElementById("end");
    temp.classList.add("unit");
    main.appendChild(temp.content.cloneNode(true));
}

function adjustHeight(n) {
    // n = no. of units
    let height;
    if (n <= 7) {
        height = 900 / n;
    } else {
        height = 300 / Math.ceil(Math.exp((Math.log(n) - Math.log(3)) / 2));
    }
    let units = main.children;
    for (u in units) {
        if(units[u].style){units[u].style.height = `calc(${height}px - 3%)`};
    }
}

function changeDist() {
    let preset = document.getElementById("presets").value;
    let probs = document.getElementsByClassName("slider");
    if (preset == "uniform") {
        for (let i = 0; i < 12; i++) {
            probs[i].value = 500;
        }
    } else if (preset == "gaussian") {
        probs[0].value = 44;
        probs[1].value = 135;
        probs[2].value = 325;
        probs[3].value = 607;
        probs[4].value = 882;
        probs[5].value = 1000;
        probs[6].value = 1000;
        probs[7].value = 882;
        probs[8].value = 607;
        probs[9].value = 325;
        probs[10].value = 135;
        probs[11].value = 44;
    } else if (preset == "natural") {
        probs[0].value = 500;
        probs[1].value = 0;
        probs[2].value = 500;
        probs[3].value = 0;
        probs[4].value = 500;
        probs[5].value = 0;
        probs[6].value = 500;
        probs[7].value = 500;
        probs[8].value = 0;
        probs[9].value = 500;
        probs[10].value = 0;
        probs[11].value = 500;
    }
}

function custom() {
    document.getElementById("presets").value = "custom";
}

document.addEventListener("keydown", function(event) {
    switch(event.key) {
        case " ":
            event.preventDefault();
            generate();
            break;
    }
});